...
class PDFCore extends PDF_PageGroupCore {
	...
	protected function _initPDFFonts() {
		...
		foreach ( $languages as $language ) {
			...
			self::$_pdfparams [$isoCode] = array ('encoding' => (isset ( $conf ['PS_PDF_ENCODING_' . $isoCode] ) and $conf ['PS_PDF_ENCODING_' . $isoCode] == true) ? $conf ['PS_PDF_ENCODING_' . $isoCode] : 'iso-8859-1', 'font' => (isset ( $conf ['PS_PDF_FONT_' . $isoCode] ) and $conf ['PS_PDF_FONT_' . $isoCode] == true) ? $conf ['PS_PDF_FONT_' . $isoCode] : 'helvetica' );
		}
	...
	*/
	private function _getCompleteUSAddressFormat($conf) {
		...
		$completeAddressShop = $shopCity . ((! empty ( $shopState ) && ! empty ( $shopCity )) ? ', ' . $shopState . ((! empty ( $shopZipcode )) ? ' ' : '') : ((! empty ( $shopState )) ? $shopState . ((! empty ( $shopZipcode )) ? ' ' : '') : ((! empty ( $shopCity ) && ! empty ( $shopZipcode )) ? ' ' : ''))) . $shopZipcode;
	...
	*/
	private function _builMerchantFooterDetail($conf) {
		...
		if (Country::getIsoById ( Configuration::get ( 'PS_COUNTRY_DEFAULT' ) ) == 'US') {
			...
			$footerText = $conf ['PS_SHOP_NAME_UPPER'] . (! empty ( $conf ['PS_SHOP_ADDR1'] ) ? ' - ' . self::l ( 'Headquarters:' ) . ' ' . $conf ['PS_SHOP_ADDR1'] . (! empty ( $conf ['PS_SHOP_ADDR2'] ) ? ' ' . $conf ['PS_SHOP_ADDR2'] : '') . ' ' . $conf ['PS_SHOP_CODE'] . ' ' . $conf ['PS_SHOP_CITY'] . ((isset ( $conf ['PS_SHOP_STATE'] ) and ! empty ( $conf ['PS_SHOP_STATE'] )) ? (', ' . $conf ['PS_SHOP_STATE']) : '') . ' ' . $conf ['PS_SHOP_COUNTRY'] : '') . "\n" . (! empty ( $conf ['PS_SHOP_DETAILS'] ) ? self::l ( 'Details:' ) . ' ' . $conf ['PS_SHOP_DETAILS'] . ' - ' : '') . (! empty ( $conf ['PS_SHOP_PHONE'] ) ? self::l ( 'PHONE:' ) . ' ' . $conf ['PS_SHOP_PHONE'] : '');
		}
	...
	public function Footer() {
		$arrayConf = array ('PS_SHOP_NAME', 'PS_SHOP_ADDR1', 'PS_SHOP_ADDR2', 'PS_SHOP_CODE', 'PS_SHOP_CITY', 'PS_SHOP_COUNTRY', 'PS_SHOP_COUNTRY_ID', 'PS_SHOP_DETAILS', 'PS_SHOP_PHONE', 'PS_SHOP_STATE' );
	...
	public function ProdTabHeader($delivery = false) {
		if (! $delivery) {
			$header = array (array (self::l ( 'Description' ), 'L' ), array (self::l ( 'Reference' ), 'L' ), array (self::l ( 'U. price' ), 'R' ), array (self::l ( 'Qty' ), 'C' ), array (self::l ( 'Total' ), 'R' ) );
