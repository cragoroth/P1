...
class CMSCategoryCore extends ObjectModel {
	...
	public static function recurseCMSCategory($categories, $current, $id_cms_category = 1, $id_selected = 1, $is_html = 0) {
		$html = '<option value="' . $id_cms_category . '"' . (($id_selected == $id_cms_category) ? ' selected="selected"' : '') . '>' . str_repeat ( '&nbsp;', $current ['infos'] ['level_depth'] * 5 ) . self::hideCMSCategoryPosition ( stripslashes ( $current ['infos'] ['name'] ) ) . '</option>';
		if ($is_html == 0)
			echo $html;
	...
	public function updatePosition($way, $position) {
		...
		return (Db::getInstance ()->Execute ( '
			...
			AND `id_parent`=' . ( int ) ($movedCategory ['id_parent']) ) and Db::getInstance ()->Execute ( '
			UPDATE `' . _DB_PREFIX_ . 'cms_category`
