...
class CMSCore extends ObjectModel {
	...
	public function updatePosition($way, $position) {
		...
		return (Db::getInstance ()->Execute ( '
			...
			WHERE `position` 
			' . ($way ? '> ' . ( int ) ($movedCms ['position']) . ' AND `position` <= ' . ( int ) ($position) : '< ' . ( int ) ($movedCms ['position']) . ' AND `position` >= ' . ( int ) ($position)) . '
			AND `id_cms_category`=' . ( int ) ($movedCms ['id_cms_category']) ) and Db::getInstance ()->Execute ( '
			UPDATE `' . _DB_PREFIX_ . 'cms`
	...
	public static function getCMSPages($id_lang = NULL, $id_cms_category = NULL, $active = true) {
		...
		SELECT *
		FROM `' . _DB_PREFIX_ . 'cms` c
		JOIN `' . _DB_PREFIX_ . 'cms_lang` l ON (c.id_cms = l.id_cms)' . (isset ( $id_cms_category ) ? 'WHERE `id_cms_category` = ' . ( int ) ($id_cms_category) : '') . ($active ? ' AND c.`active` = 1 ' : '') . '
