...
class ProductCore extends ObjectModel {
	...
	protected $fieldsValidateLang = array ('meta_description' => 'isGenericName', 'meta_keywords' => 'isGenericName', 'meta_title' => 'isGenericName', 'link_rewrite' => 'isLinkRewrite', 'name' => 'isCatalogName', 'description' => 'isString', 'description_short' => 'isString', 'available_now' => 'isGenericName', 'available_later' => 'IsGenericName' );
	...
	public function updatePosition($way, $position) {
		...
		return (Db::getInstance ()->Execute ( '
			...
			WHERE `position`
			' . ($way ? '> ' . ( int ) ($movedProduct ['position']) . ' AND `position` <= ' . ( int ) ($position) : '< ' . ( int ) ($movedProduct ['position']) . ' AND `position` >= ' . ( int ) ($position)) . '
			AND `id_category`=' . ( int ) ($movedProduct ['id_category']) ) and Db::getInstance ()->Execute ( '
			UPDATE `' . _DB_PREFIX_ . 'category_product`
	...
	public function deleteTags() {
		return (Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'product_tag` WHERE `id_product` = ' . ( int ) ($this->id) ) and Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'tag` WHERE `id_tag` NOT IN (SELECT `id_tag` FROM `' . _DB_PREFIX_ . 'product_tag`)' ));
	}
	...
	public function addProductAttribute($price, $weight, $unit_impact, $ecotax, $quantity, $id_images, $reference, $supplier_reference, $ean13, $default, $location = NULL, $upc = NULL) {
		...
		Db::getInstance ()->AutoExecute ( _DB_PREFIX_ . 'product_attribute', array ('id_product' => ( int ) ($this->id), 'price' => ( float ) ($price), 'ecotax' => ( float ) ($ecotax), 'quantity' => 0, 'weight' => ($weight ? ( float ) ($weight) : 0), 'unit_price_impact' => ($unit_impact ? ( float ) ($unit_impact) : 0), 'reference' => pSQL ( $reference ), 'supplier_reference' => pSQL ( $supplier_reference ), 'location' => pSQL ( $location ), 'ean13' => pSQL ( $ean13 ), 'upc' => pSQL ( $upc ), 'default_on' => ( int ) ($default) ), 'INSERT' );
	...
	public function addProductAttributeMultiple($attributes, $setDefault = true) {
		...
		return (array_map ( create_function ( '$elem', 'return $elem[\'id_product_attribute\'];' ), Db::getInstance ()->ExecuteS ( '
	...
	public function setDefaultAttribute($id_product_attribute) {
		...
		SET `default_on` = 1
		WHERE `id_product` = ' . ( int ) $this->id . '
		AND `id_product_attribute` = ' . ( int ) $id_product_attribute ) && Db::getInstance ()->Execute ( '
	...
	public function deleteCustomization() {
		return Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'customization_field` WHERE `id_product` = ' . ( int ) ($this->id) ) && Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'customization_field_lang` WHERE `id_customization_field` NOT IN (SELECT id_customization_field FROM `' . _DB_PREFIX_ . 'customization_field`)' );
	}
	...
	public function deleteSearchIndexes() {
		return Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'search_index` WHERE `id_product` = ' . ( int ) ($this->id) ) && Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'search_word` WHERE `id_word` NOT IN (SELECT id_word FROM `' . _DB_PREFIX_ . 'search_index`)' );
	}
	...
	public static function duplicateAccessories($id_product_old, $id_product_new) {
		...
		foreach ( $result as $row ) {
			$data = array ('id_product_1' => ( int ) ($id_product_new), 'id_product_2' => ( int ) ($row ['id_product_2']) );
