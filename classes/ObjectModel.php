...
abstract class ObjectModelCore {
	...
	public function validateFields($die = true, $errorReturn = false) {
		...
		foreach ( $fieldsRequired as $field )
			if (Tools::isEmpty ( $this->{$field} ) and (! is_numeric ( $this->{$field} ))) {
				if ($die)
					die ( Tools::displayError () . ' (' . get_class ( $this ) . ' -> ' . $field . ' is empty)' );
		...
		foreach ( $this->fieldsSize as $field => $size )
			if (isset ( $this->{$field} ) and Tools::strlen ( $this->{$field} ) > $size) {
				if ($die)
					die ( Tools::displayError () . ' (' . get_class ( $this ) . ' -> ' . $field . ' Length ' . $size . ')' );
		...
		foreach ( $this->fieldsValidate as $field => $method )
			...
			elseif (! empty ( $this->{$field} ) and ! call_user_func ( array ('Validate', $method ), $this->{$field} )) {
				if ($die)
					die ( Tools::displayError () . ' (' . get_class ( $this ) . ' -> ' . $field . ' = ' . $this->{$field} . ')' );
		...
		foreach ( $this->fieldsValidateLang as $fieldArray => $method ) {
			...
			foreach ( $this->{$fieldArray} as $k => $value )
				...
				elseif (! empty ( $value ) and ! call_user_func ( array ('Validate', $method ), $value )) {
					if ($die)
						die ( Tools::displayError ( 'The following field is invalid according to the validate method ' ) . '<b>' . $method . '</b>:<br/> (' . get_class ( $this ) . '->' . $fieldArray . ' = ' . $value . ' ' . Tools::displayError ( 'for language' ) . ' ' . $k . ')' );
	...
	public function deleteImage() {
		...
		if (file_exists ( _PS_TMP_IMG_DIR_ . $this->table . '_' . $this->id . '.' . $this->image_format ) && ! unlink ( _PS_TMP_IMG_DIR_ . $this->table . '_' . $this->id . '.' . $this->image_format ))
			return false;
		if (file_exists ( _PS_TMP_IMG_DIR_ . $this->table . '_mini_' . $this->id . '.' . $this->image_format ) && ! unlink ( _PS_TMP_IMG_DIR_ . $this->table . '_mini_' . $this->id . '.' . $this->image_format ))
			return false;
