...
class OrderCore extends ObjectModel {
	...
	public function getDiscounts($details = false) {
		return Db::getInstance ( _PS_USE_SQL_SLAVE_ )->ExecuteS ( '
		SELECT *
		FROM `' . _DB_PREFIX_ . 'order_discount` od ' . ($details ? 'LEFT JOIN `' . _DB_PREFIX_ . 'discount` d ON (d.`id_discount` = od.`id_discount`)' : '') . '
		WHERE od.`id_order` = ' . ( int ) ($this->id) );
	}
	...
	public static function getOrdersIdByDate($date_from, $date_to, $id_customer = NULL, $type = NULL) {
		...
		WHERE DATE_ADD(date_upd, INTERVAL -1 DAY) <= \'' . pSQL ( $date_to ) . '\' AND date_upd >= \'' . pSQL ( $date_from ) . '\'' . ($type ? ' AND ' . pSQL ( strval ( $type ) ) . '_number != 0' : '') . ($id_customer ? ' AND id_customer = ' . ( int ) ($id_customer) : '');
	...
	*/
	...
	public static function getOrdersIdInvoiceByDate($date_from, $date_to, $id_customer = NULL, $type = NULL) {
		...
		SELECT `id_order`
		FROM `' . _DB_PREFIX_ . 'orders`
		WHERE DATE_ADD(invoice_date, INTERVAL -1 DAY) <= \'' . pSQL ( $date_to ) . '\' AND invoice_date >= \'' . pSQL ( $date_from ) . '\'' . ($type ? ' AND ' . pSQL ( strval ( $type ) ) . '_number != 0' : '') . ($id_customer ? ' AND id_customer = ' . ( int ) ($id_customer) : '') . ' ORDER BY invoice_date ASC' );
	...
	public function setInvoice() {
		...
		else if (version_compare ( Db::getInstance ()->getServerVersion (), 5, '<' )) {
