...
class UpgraderCore {
		...
		// if we use the autoupgrade process, we will never refresh it
			...
			if ($force || ($last_check < time () - (3600 * Upgrader::DEFAULT_CHECK_VERSION_DELAY_HOURS))) {
				...
				if ($feed = @simplexml_load_file ( $this->rss_version_link )) {
					...
					$config_last_version = array ('name' => $this->version_name, 'num' => $this->version_num, 'link' => $this->link, 'md5' => $this->md5, 'autoupgrade' => $this->autoupgrade, 'autoupgrade_module' => $this->autoupgrade_module, 'changelog' => $this->changelog, 'desc' => $this->desc );
					if (class_exists ( 'Configuration' )) {
	...
	protected function addChangedFile($path) {
		...
		else if (strpos ( $path, '/en.php' ) !== false || strpos ( $path, '/fr.php' ) !== false || strpos ( $path, '/es.php' ) !== false || strpos ( $path, '/it.php' ) !== false || strpos ( $path, '/de.php' ) !== false || strpos ( $path, 'translations/' ) !== false)
