...
class TaxCore extends ObjectModel {
	...
	public static function getTaxes($id_lang = false, $active = 1) {
		...
		' . (( int ) ($id_lang) ? 'LEFT JOIN `' . _DB_PREFIX_ . 'tax_lang` tl ON (t.`id_tax` = tl.`id_tax` AND tl.`id_lang` = ' . ( int ) ($id_lang) . ')' . ($active == 1 ? 'WHERE t.`active` = 1' : '') . '
		ORDER BY `name` ASC' : '') );
	}
	...
	public static function getTaxIdByRate($rate, $active = 1) {
		...
		$tax = Db::getInstance ( _PS_USE_SQL_SLAVE_ )->getRow ( '
			...
			WHERE t.`rate` = ' . ( float ) ($rate) . ($active == 1 ? ' AND t.`active` = 1' : '') );
