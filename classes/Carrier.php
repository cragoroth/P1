...
class CarrierCore extends ObjectModel {
	...
	protected $webserviceParameters = array ('fields' => array ('id_tax_rules_group' => array (), 'deleted' => array (), 'is_module' => array () ) );
	...
	public static function getCarriers($id_lang, $active = false, $delete = false, $id_zone = false, $ids_group = NULL, $modules_filters = self::PS_CARRIERS_ONLY) {
		...
		$sql = '
			...
			WHERE c.`deleted` ' . ($delete ? '= 1' : ' = 0') . ($active ? ' AND c.`active` = 1' : '') . ($id_zone ? ' AND cz.`id_zone` = ' . ( int ) ($id_zone) . '
			AND z.`active` = 1 ' : ' ');
	...
	public static function getDeliveredCountries($id_lang, $activeCountries = false, $activeCarriers = false, $containStates = NULL) {
		...
		$result = Db::getInstance ( _PS_USE_SQL_SLAVE_ )->ExecuteS ( '
			...
			INNER JOIN (`' . _DB_PREFIX_ . 'carrier_zone` cz INNER JOIN `' . _DB_PREFIX_ . 'carrier` cr ON ( cr.id_carrier = cz.id_carrier AND cr.deleted = 0 ' . ($activeCarriers ? 'AND cr.active = 1) ' : ') ') . '
	...
	public static function getCarriersForOrder($id_zone, $groups = NULL) {
		...
		foreach ( $result as $k => $row ) {
			...
			if ($shippingMethod != Carrier::SHIPPING_METHOD_FREE) {
				...
				if (($shippingMethod == Carrier::SHIPPING_METHOD_WEIGHT and $carrier->getMaxDeliveryPriceByWeight ( $id_zone ) === false) or ($shippingMethod == Carrier::SHIPPING_METHOD_PRICE and $carrier->getMaxDeliveryPriceByPrice ( $id_zone ) === false)) {
					unset ( $result [$k] );
					continue;
				}
		...
		// Get only carriers that have a range compatible with cart
					if (($shippingMethod == Carrier::SHIPPING_METHOD_WEIGHT and (! Carrier::checkDeliveryPriceByWeight ( $row ['id_carrier'], $cart->getTotalWeight (), $id_zone ))) or ($shippingMethod == Carrier::SHIPPING_METHOD_PRICE and (! Carrier::checkDeliveryPriceByPrice ( $row ['id_carrier'], $cart->getOrderTotal ( true, Cart::BOTH_WITHOUT_SHIPPING ), $id_zone, $cart->id_currency )))) {
						unset ( $result [$k] );
						continue;
					}
				}
