...
class AddressFormatCore extends ObjectModel {
	...
	public static $requireFormFieldsList = array ('firstname', 'name', 'address1', 'city', 'postcode', 'Country:name', 'State:name' );
	
	public static $forbiddenPropertyList = array ('deleted', 'date_add', 'other', 'alias', 'secure_key', 'note', 'newsletter', 'ip_registration_newsletter', 'newsletter_date_add', 'optin', 'passwd', 'last_passwd_gen', 'active', 'is_guest', 'date_upd', 'years', 'days', 'months', 'description', 'meta_description', 'short_description', 'link_rewrite', 'meta_title', 'meta_keywords', 'display_tax_label', 'need_zip_code', 'contains_states', 'call_prefixes', 'call_prefix' );
	...
	private function _checkValidateClassField($className, $fieldName, $isIdField) {
		...
		if (! class_exists ( $className ))
			$this->_errorFormatList [] = Tools::displayError ( 'This class name doesn\'t exist' ) . ': ' . $className;
		else {
			$obj = new $className ();
			...
			foreach ( $publicProperties as $property ) {
				...
				if (($propertyName == $fieldName) && ($isIdField || (! preg_match ( '#id|id_\w#', $propertyName ))))
					$isValide = true;
			}
			
			if (! $isValide)
				$this->_errorFormatList [] = Tools::displayError ( 'This property doesn\'t exist in the class or is forbidden' ) . ': ' . $className . ': ' . $fieldName;
		...
		if ($associationName = explode ( ':', $patternName )) {
			...
			elseif ($totalNameUsed == 1) {
				...
				if (in_array ( $associationName [0], self::$forbiddenPropertyList ) || ! $this->_checkValidateClassField ( 'Address', $associationName [0], false ))
					$this->_errorFormatList [] = Tools::displayError ( 'This name isn\'t allowed' ) . ': ' . $associationName [0];
					...
					if (is_array ( $patternsName )) {
						foreach ( $patternsName as $patternName ) {
							if (! in_array ( $patternName, $usedKeyList )) {
								...
								$this->_errorFormatList [] = Tools::displayError ( 'This key is used too many times (once allowed)' ) . ': ' . $patternName;
						}
					}
			}
					...
					foreach ( $keyList as $pattern )
						if ($associateName = explode ( ':', $pattern )) {
							...
							if ($totalName == 1 && isset ( $address->{$associateName [0]} ))
								...
								if (($totalName == 2) && class_exists ( $associateName [0] ) && Tools::property_exists ( $associateName [0], $associateName [1] ) && Tools::property_exists ( $address, 'id_' . strtolower ( $associateName [0] ) )) {
									...
									if ($temporyObject [$associateName [0]])
										$tab [$pattern] = (is_array ( $temporyObject [$associateName [0]]->{$associateName [1]} )) ? ((isset ( $temporyObject [$associateName [0]]->{$associateName [1]} [(isset ( $cookie ) ? $cookie->id_lang : Configuration::get ( 'PS_LANG_DEFAULT' ))] )) ? $temporyObject [$associateName [0]]->{$associateName [1]} [(isset ( $cookie ) ? $cookie->id_lang : Configuration::get ( 'PS_LANG_DEFAULT' ))] : '') : $temporyObject [$associateName [0]]->{$associateName [1]};
								}
							}
						}
