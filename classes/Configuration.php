...
class ConfigurationCore extends ObjectModel {
	...
	public static function deleteByName($key) {
		...
		if (Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'configuration_lang` WHERE `id_configuration` = (SELECT `id_configuration` FROM `' . _DB_PREFIX_ . 'configuration` WHERE `name` = \'' . pSQL ( $key ) . '\')' ) and Db::getInstance ()->Execute ( 'DELETE FROM `' . _DB_PREFIX_ . 'configuration` WHERE `name` = \'' . pSQL ( $key ) . '\'' )) {
	...
	public static function updateValue($key, $values, $html = false) {
		...
		if (! is_array ( $values )) {
			if (Configuration::get ( $key ) !== false) {
				...
				$result = $db->AutoExecute ( _DB_PREFIX_ . 'configuration', array ('value' => $values, 'date_upd' => date ( 'Y-m-d H:i:s' ) ), 'UPDATE', '`name` = \'' . pSQL ( $key ) . '\'', true, true );
